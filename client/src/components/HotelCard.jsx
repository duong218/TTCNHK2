...
const HotelCard = ({room,index}) => {
    ...
    return (
        <Link to={'/rooms/' + room._id} onClick={() => scrollTo(0, 0)} key={room._id} className='relative max-w-70 w-full rounded-xl overflow-hidden bg-white text-gray-500/90 shadow-[0px_4px_4px_rgba(0,0,0,0.05)]'>
            ...
            {index % 2 === 0 && <p className='px-3 py-1 absolute top-3 left-3 text-xs bg-white text-gray-800 font-medium rounded-full'>Bán chạy nhất</p>}
            <div className='p-4 pt-5'>
                ...
                <div className='flex items-center justify-between mt-4'>
                    ...
                    <button className='px-4 py-2 text-sm font-medium border border-gray-300 rounded hover:bg-gray-50 transition-all cursor-pointer'>Đặt ngay</button>
                </div>
            </div>
        </Link>
    )
