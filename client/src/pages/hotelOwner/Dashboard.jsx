...
const Dashboard = () => {
    ...
    return (
        <div>
            <Title align='left' font='outfit' title='Bảng điều khiển' subTitle='Theo dõi danh sách phòng của bạn, ghi lại các đặt phòng và phân tích doanh thu—tất cả trong một nơi. Cập nhật thông tin theo thời gian thực để đảm bảo hoạt động suôn sẻ.' />
            <div className='flex gap-4 my-8'>
                <div className='bg-primary/3 border border-primary/10 rounded flex p-4 pr-8'>
                    <img className='max-sm:hidden h-10' src={assets.totalBookingIcon} alt="" />
                    <div className='flex flex-col sm:ml-4 font-medium'>
                        <p className='text-blue-500 text-lg'>Tổng số đặt chỗ</p>
                                ...
                                <tr key={index}>
                                    ...
                                    <td className='py-3 px-4  border-t border-gray-300 flex'>
                                        <button className={`py-1 px-3 text-xs rounded-full mx-auto ${item.isPaid ? "bg-green-200 text-green-600" : "bg-amber-200 text-yellow-600"}`}>
                                            {item.isPaid ? "Hoàn thành" : "Đang chờ xử lý"}
                                        </button>
                                    </td>
                                </tr>
                            ))
